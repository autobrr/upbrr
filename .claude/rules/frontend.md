...
- Working directory for all frontend commands: `gui/frontend/`
- Package manager: pnpm 10 (`pnpm install --frozen-lockfile`)
- TypeScript strict mode, ESLint clean — no rule weakening or type error bypasses
- Validate with: `pnpm run lint` and `pnpm run typecheck`
- For build/config changes also run: `pnpm run build`
