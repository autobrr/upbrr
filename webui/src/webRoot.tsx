...
import { authClient, initializeWebClient, updateWebCSRFToken, withBasePath } from "./api/client";
...
export default function WebRoot() {
  ...
  // in the query string. Since #273 removed the Wails shell, this root only
  // ever renders in a browser, so the old browser-mode guard is gone.
  const [error, setError] = useState<string>(() => readOIDCError());
