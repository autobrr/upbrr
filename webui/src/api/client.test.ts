...
describe("web client", () => {
  ...
  it("preserves tracker auth status fields from browser app routes", async () => {
    ...
    const result = await trackerAuthClient.getStatus("TEST");

    expect(result).toEqual(status);
    expect(fetchMock).toHaveBeenCalledWith(
      "/api/app/GetTrackerAuthStatus",
      expect.objectContaining({
        body: JSON.stringify({ Tracker: "TEST" }),
      }),
    );
  });
  ...
  it("preserves tracker auth app route error messages", async () => {
    ...
    await expect(trackerAuthClient.test("TEST")).rejects.toThrow("tracker auth: validation failed");
  });
