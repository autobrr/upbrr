...
export type ExternalIdentity = {
  ...
  TVmazeID: number;
  MALID: number;
  Category: "unknown" | "movie" | "tv" | string;
  Provenance: Record<string, string>;
  Overrides: Record<string, string>;
  Conflict: "none" | "unverified" | string;
  Resolution: {
...
/** Exact identity for one immutable prepared-release generation. */
...
export type PreparedRelease = {
  ...
  Assessments: {
    MediaInfoUniqueID: "unknown" | "not_applicable" | "present" | "missing" | string;
    MediaInfoEncodeSettings: "unknown" | "not_applicable" | "present" | "missing" | string;
    Naming: {
      Status: "unknown" | "complete" | "incomplete" | string;
      Missing: string[];
    };
  };
  PreparedAt: string;
...
export type PlaylistInstruction = {
  Set: boolean;
  Selected: string[];
  UseAll: boolean;
};
...
export type TrackerAuthCapability = {
  /** Normalized tracker code used in tracker auth API calls. */
  trackerID: string;
  displayName: string;
 ...
 *
 * Mirrors the shared Go API contract; browser routes should
 * preserve every field so WebUI surfaces show the same status and remediation.
 */
...
export type IdentityProvider = "tmdb" | "imdb" | "tvdb" | "tvmaze" | "mal";
...
/** Canonical prepared-release display entry with one provider-matched detail payload. */
export type ProviderDisplay =
  | (ProviderDisplayBase & { Provider: "tmdb"; Details: { TMDB: TMDBMetadata } })
  | (ProviderDisplayBase & { Provider: "imdb"; Details: { IMDB: IMDBMetadata } })
  | (ProviderDisplayBase & { Provider: "tvdb"; Details: { TVDB: TVDBMetadata } })
  | (ProviderDisplayBase & { Provider: "tvmaze"; Details: { TVmaze: TVmazeMetadata } })
...
/** Advisory terminal and non-terminal states for one image-host attempt. */
export type ImageUploadProgressStatus = "running" | "completed" | "failed";

/** Absolute image-host progress snapshot correlated to one upload command. */
export type ImageUploadProgressUpdate = {
  /** Binds the update to one image-upload command. */
  correlationID: string;
  /** Stable host/scope row identifier within the command. */
  attemptID: string;
  /** Normalized configured image-host name. */
  host: string;
  /** Global or tracker-owned persistence scope. */
  usageScope: string;
  /** Trackers served by this host attempt. */
  trackers: string[];
  /** Whether this attempt follows an earlier host failure. */
  fallback: boolean;
  /** Succeeded, failed, and reused images in terminal states. */
  completed: number;
  /** Selected images required on this host. */
  total: number;
  /** Successful network uploads in this attempt. */
  succeeded: number;
  /** Network uploads that did not produce a usable link. */
  failed: number;
  /** Images satisfied by existing persisted host records. */
  reused: number;
  /** Latest advisory state for this host attempt. */
  status: ImageUploadProgressStatus;
  /** Frontend-safe detail for the latest transition. */
  message: string;
  /** UTC RFC3339 timestamp assigned by the WebUI boundary. */
  timestamp: string;
};
