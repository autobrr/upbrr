...
//
// The trust model that follows from this is worth stating plainly: every
// identity that completes the flow against the configured client lands on this
// one account, matching username or not, and there is no narrower permission
// tier beneath it. Operators must scope the provider application to trusted
// users; the README says so under "Single Sign-On (OIDC)".
func (s *Server) resolveOIDCAccount(identity string) (string, error) {
	...
	if !exists {
		username, provisionErr := s.provisionOIDCRecord(identity)
		if provisionErr == nil {
			return username, nil
		}
		// A second first-login can create the account between the Exists check
		// above and Bootstrap below, and Bootstrap then refuses. That collision
		// is the one recoverable failure here: if a record is present now, adopt
		// it rather than failing an otherwise valid login. Any other error
		// still propagates.
		created, existsErr := wrapWebResult(s.auth.Exists())
		if existsErr != nil || !created {
			return "", provisionErr
		}
		s.logInfof("web: oidc provisioning lost a race with a concurrent login; using the account that won")
	}
