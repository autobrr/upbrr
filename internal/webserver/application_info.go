...
// seconds and returns unavailable status when ctx or provider is nil, including
// an interface containing a typed nil pointer.
func CurrentApplicationInfo(ctx context.Context, provider api.DVDMenuCapabilityProvider) api.ApplicationInfo {
	...
	if !CapabilityAvailable(provider) {
		return info
	}
	if ctx == nil {
		return info
