...
type fakeIDP struct {
	...
	key    *rsa.PrivateKey
	// subject and preferredUsername shape the identity claims. They are set
	// before the server starts and never mutated, so they need no locking.
	subject           string
	preferredUsername string
	// pkceSupported controls the advertised code_challenge_methods_supported.
	// Also set before the server starts.
	pkceSupported bool

	// mu guards the two fields the test goroutine and the server's handler
...
func (f *fakeIDP) currentNonce() string {
	f.mu.Lock()
	defer f.mu.Unlock()
	return f.nonce
}

func (f *fakeIDP) setLastCodeVerifier(verifier string) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.lastCodeVerifier = verifier
}

func (f *fakeIDP) codeVerifier() string {
	f.mu.Lock()
	defer f.mu.Unlock()
	return f.lastCodeVerifier
}
...
func newFakeIDP(t *testing.T) *fakeIDP {
	...
	mux.HandleFunc("/token", func(w http.ResponseWriter, r *http.Request) {
		...
		idp.setLastCodeVerifier(r.Form.Get("code_verifier"))
		// Signing happens on this handler goroutine, so it must not call
		// t.Fatalf — that would run runtime.Goexit on the wrong goroutine and
		// the test would not fail cleanly. Report it as a server error instead;
		// the client-side assertions then fail with the response in hand.
...
// signIDToken returns an error rather than failing the test directly: it runs on
// the token endpoint's handler goroutine, where the testing helpers must not be
// used.
func (f *fakeIDP) signIDToken() (string, error) {
...
func startFlow(t *testing.T, srv *Server, idp *fakeIDP) (string, *http.Cookie) {
	...
	idp.setNonce(redirect.Query().Get("nonce"))
	if idp.currentNonce() == "" {
...
func TestOIDCNonceMismatchIsRejected(t *testing.T) {
	...
	state, cookie := startFlow(t, srv, idp)
	idp.setNonce("nonce-from-a-different-flow")
...
func TestOIDCScopeListAlwaysRequestsOpenID(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "keeps openid once",
			input: "openid openid profile",
