...
func TestReleaseWorkflowOperationClassification(t *testing.T) {
	t.Parallel()
	tests := []struct {
		command releaseworkflow.Command
		want    api.OperationKind
	}{
		{command: releaseworkflow.PrepareReleaseCommand{}, want: api.OperationKindPreparation},
		{command: releaseworkflow.CheckDuplicatesCommand{}, want: api.OperationKindDuplicateCheck},
		{command: releaseworkflow.CaptureMediaCommand{}, want: api.OperationKindMedia},
		{command: releaseworkflow.GenerateDescriptionsCommand{}, want: api.OperationKindDescription},
		{command: releaseworkflow.DryRunUploadsCommand{}, want: api.OperationKindUploadDryRun},
		{command: releaseworkflow.ExecuteUploadsCommand{}, want: api.OperationKindUploadExecute},
		{command: releaseworkflow.RetryFailedUploadsCommand{}, want: api.OperationKindUploadExecute},
	}
	for _, test := range tests {
		if got := releaseWorkflowOperation(test.command); got != test.want {
			t.Fatalf("releaseWorkflowOperation(%T) = %s, want %s", test.command, got, test.want)
		}
	}
}
