...
func TestWorkflowUploadPlanAppliesSkipIfRehashAndQueuesSharedBaseUsers(t *testing.T) {
	...
	defer func() { _ = execution.Release() }()
	if torrents.calls != 1 || !slices.Equal(torrents.subject.SkipIfRehashTrackers, []string{"TEST"}) {
		t.Fatal("torrent preparation did not receive the TEST skip policy exactly once")
	}
	...
	if plan.Status != api.StageStatusReady || len(plan.Trackers) != 3 || plan.Trackers[1].TrackerID != "TEST" ||
		plan.Trackers[1].Status != api.StageStatusSkipped || plan.Trackers[1].Eligible {
		t.Fatalf("TEST skip plan = %#v", plan.Trackers)
	}
}
