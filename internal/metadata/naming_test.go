...
func TestBuildReleaseNameMovieWebDL(t *testing.T) {
	...
	if result.GeneratedName == nil {
		t.Fatal("expected generated name document")
	}
	if err := result.GeneratedName.Validate(); err != nil {
		t.Fatalf("generated document validation: %v", err)
	}
	if got := result.GeneratedName.Render(); got != (api.ReleaseNameVariant{
		NameNoTag: result.NameNoTag,
		Name:      result.Name,
		CleanName: result.CleanName,
	}) {
		t.Fatalf("generated document rendered %#v, result %#v", got, result)
	}
	if hdr, ok := result.GeneratedName.Component(api.NameRoleHDR); !ok || hdr.Value != "HDR" {
		t.Fatalf("HDR component = %#v, found=%t", hdr, ok)
	}
	if format, ok := result.GeneratedName.Component(api.NameRoleVideoFormat); !ok || format.Value != "WEB-DL" {
		t.Fatalf("video format component = %#v, found=%t", format, ok)
	}
	if encode, ok := result.GeneratedName.Component(api.NameRoleVideoEncode); !ok || encode.Value != "H.265" {
		t.Fatalf("video encode component = %#v, found=%t", encode, ok)
	}
}

func TestMergeGeneratedNameAvailabilityRetainsClearedManualValue(t *testing.T) {
	t.Parallel()

	available := &api.ReleaseNameDocument{Version: api.ReleaseNameDocumentVersionV1, Components: []api.ReleaseNameComponent{
		{
	...
	mergeGeneratedNameAvailability(document, available, &preparationstate.State{ReleaseNameOverrides: api.ReleaseNameOverrides{NoEdition: new(true)}}, api.ReleaseNameRequest{})

	edition, ok := document.Component(api.NameRoleEdition)
	if !ok || edition.AvailableValue != "Director's Cut" || !edition.Manual || edition.Present {
		t.Fatalf("edition component = %#v, found=%t", edition, ok)
	}
}

func TestBuildReleaseNameSeparatesAudioMarkersWithoutTouchingTitle(t *testing.T) {
	t.Parallel()
	...
	if got, want := result.NameNoTag, "Dubbed Story 2026 1080p NF WEB-DL Dubbed DD+5.1 Dual-Audio H.265"; got != want {
		t.Fatalf("NameNoTag = %q, want %q", got, want)
	}
	if title, ok := result.GeneratedName.Component(api.NameRoleTitle); !ok || title.Value != "Dubbed Story" {
		t.Fatalf("title component = %#v, found=%t", title, ok)
	}
	if dubbed, ok := result.GeneratedName.Component(api.NameRoleDubbed); !ok || !dubbed.Present || dubbed.Value != "Dubbed" {
		t.Fatalf("dubbed component = %#v, found=%t", dubbed, ok)
	}
	if dual, ok := result.GeneratedName.Component(api.NameRoleDualAudio); !ok || !dual.Present || dual.Value != "Dual-Audio" {
		t.Fatalf("dual audio component = %#v, found=%t", dual, ok)
	}
	if audio, ok := result.GeneratedName.Component(api.NameRoleAudio); !ok || audio.Value != "DD+5.1" {
		t.Fatalf("audio component = %#v, found=%t", audio, ok)
	}
}

func TestBuildReleaseNamePreservesLeadingDualAudioMarker(t *testing.T) {
	t.Parallel()
	...
	if got, want := result.NameNoTag, "Example Release 2026 1080p NF WEB-DL Dual-Audio DD+5.1 H.265"; got != want {
		t.Fatalf("NameNoTag = %q, want %q", got, want)
	}
	dual, ok := result.GeneratedName.Component(api.NameRoleDualAudio)
	if !ok || !dual.Present || dual.Join != " " {
		t.Fatalf("dual audio component = %#v, found=%t", dual, ok)
	}
}

func TestBuildReleaseNameDoesNotFabricateAbsentDualAudioAvailability(t *testing.T) {
	t.Parallel()
	...
	dual, ok := result.GeneratedName.Component(api.NameRoleDualAudio)
	if !ok || dual.Present || dual.Value != "" || dual.AvailableValue != "" {
		t.Fatalf("absent dual audio component = %#v, found=%t", dual, ok)
	}
}

func TestBuildReleaseNameTVDVDRipRetainsSeasonOnlyAndNormalizesWhitespace(t *testing.T) {
	t.Parallel()
...
func TestBuildReleaseNameTVSeparatesSeasonEpisodeWithAttachmentFallback(t *testing.T) {
	t.Parallel()
	...
	if got, want := result.NameNoTag, "Example Show S01E023D DVD9 DD 5.1"; got != want {
		t.Fatalf("NameNoTag = %q, want %q", got, want)
	}
	season, _ := result.GeneratedName.Component(api.NameRoleSeason)
	episode, _ := result.GeneratedName.Component(api.NameRoleEpisode)
	threeD, _ := result.GeneratedName.Component(api.NameRoleThreeD)
	if episode.AttachTo[0] != api.NameRoleSeason || threeD.AttachTo[0] != api.NameRoleEpisode {
		t.Fatalf("attachment layout season=%#v episode=%#v 3D=%#v", season, episode, threeD)
	}

	request.NoSeason = true
	result = BuildReleaseName(request, api.NopLogger{})
	if got, want := result.NameNoTag, "Example Show 3D DVD9 DD 5.1"; got != want {
		t.Fatalf("NameNoTag without season = %q, want %q", got, want)
	}
}

func TestMergeGeneratedNameAvailabilityKeepsPresentAudioMarkerLayout(t *testing.T) {
	t.Parallel()
	...
	automatic := BuildReleaseName(base, api.NopLogger{}).GeneratedName
	overrides := api.ReleaseNameOverrides{NoDual: new(true), DualAudio: new(true)}
	base.Audio = applyAudioOverrides(base.Audio, overrides)
	result := BuildReleaseName(base, api.NopLogger{})
	mergeGeneratedNameAvailability(result.GeneratedName, automatic, &preparationstate.State{ReleaseNameOverrides: overrides}, base)
	if got, want := result.GeneratedName.Render(), (api.ReleaseNameVariant{
		NameNoTag: result.NameNoTag,
		Name:      result.Name,
		CleanName: result.CleanName,
	}); got != want {
		t.Fatalf("rendered document = %#v, result = %#v", got, want)
	}
	dual, _ := result.GeneratedName.Component(api.NameRoleDualAudio)
	if !dual.Present || !dual.Manual || dual.Join != " " {
		t.Fatalf("dual component = %#v", dual)
	}
}

func TestMergeGeneratedNameAvailabilityRetainsOmittedDubbedMarker(t *testing.T) {
	t.Parallel()
	...
	automatic := BuildReleaseName(base, api.NopLogger{}).GeneratedName
	base.Audio = applyAudioOverrides(base.Audio, api.ReleaseNameOverrides{NoDub: new(true)})
	final := BuildReleaseName(base, api.NopLogger{}).GeneratedName
	mergeGeneratedNameAvailability(final, automatic, &preparationstate.State{ReleaseNameOverrides: api.ReleaseNameOverrides{NoDub: new(true)}}, base)

	if got, want := final.Render().NameNoTag, "Dubbed Story 2026 1080p NF WEB-DL DD+5.1 H.265"; got != want {
		t.Fatalf("rendered name = %q, want %q", got, want)
	}
	dubbed, ok := final.Component(api.NameRoleDubbed)
	if !ok || dubbed.Present || !dubbed.Manual || dubbed.AvailableValue != "Dubbed" {
		t.Fatalf("omitted dubbed component = %#v, found=%t", dubbed, ok)
	}
	title, ok := final.Component(api.NameRoleTitle)
	if !ok || title.Value != "Dubbed Story" || title.Manual {
		t.Fatalf("title component = %#v, found=%t", title, ok)
	}
}

func TestMergeGeneratedNameAvailabilityPreservesManualFacts(t *testing.T) {
	t.Parallel()
	...
	document := &api.ReleaseNameDocument{Version: api.ReleaseNameDocumentVersionV1, Components: []api.ReleaseNameComponent{
		...
		},
		{
			...
			Value:   "Manual Episode",
			Present: true,
		},
	}}
	mergeGeneratedNameAvailability(document, nil, &preparationstate.State{EffectiveMetadata: api.EffectiveMetadata{
		TitleProvenance: api.FactProvenanceManual,
		YearProvenance:  api.FactProvenanceManual,
	}}, api.ReleaseNameRequest{ManualEpisodeTitle: true})
	for _, role := range []api.ReleaseNameRole{api.NameRoleTitle, api.NameRoleYear, api.NameRoleEpisodeTitle} {
		component, ok := document.Component(role)
		if !ok || !component.Manual {
			t.Fatalf("manual component %q = %#v, found=%t", role, component, ok)
		}
	}
}

func TestCaptureAvailableGeneratedNameRefreshesCurrentFacts(t *testing.T) {
	t.Parallel()
	...
	meta := preparationstate.State{
		...
		AvailableGeneratedName: &api.ReleaseNameDocument{Version: api.ReleaseNameDocumentVersionV1, Components: []api.ReleaseNameComponent{
			{
				Role:           api.NameRoleEdition,
				Value:          "Stale Edition",
				AvailableValue: "Stale Edition",
				Present:        true,
			},
		}},
	}
	...
	edition, ok := meta.AvailableGeneratedName.Component(api.NameRoleEdition)
	if !ok || edition.Value != "Current Edition" {
		t.Fatalf("available edition = %#v, found=%t", edition, ok)
	}
}
...
func TestBuildReleaseNameDVDDiscDoesNotRepeatDVD(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "PAL movie missing size",
			category: "MOVIE",
			title:    "Example Release",
			source:   "PAL DVD",
			want:     "Example Release 2026 PAL DVD",
		},
		{
			name:     "NTSC TV unknown size",
			category: "TV",
			title:    "Example Show",
			season:   "S01",
			source:   "NTSC DVD",
			size:     "DVD10",
			want:     "Example Show 2026 S01 NTSC DVD DVD10",
		},
		{
			name:     "other movie known size",
			category: "MOVIE",
			title:    "Example Release",
			source:   "Other",
			size:     "DVD5",
			want:     "Example Release 2026 Other DVD5",
		},
	}
