...
func TestFreshContentCorrectionFinalizesBindingAndInheritedIdentityChangeStopsCommit(t *testing.T) {
	...
	store := newMemoryStore()
	module := newTestModule(t, store, &recordingCollector{})
	input := api.PrepareInput{SourcePath: source, Instructions: api.ReleaseFactInstructions{Metadata: api.MetadataOverrides{Title: new("Manual title")}}}
	first, err := module.Prepare(t.Context(), input)
	if err != nil {
		t.Fatal(err)
	}
	binding := first.Corrections.Corrections.ContentBindings[api.CorrectionFieldMetadataTitle]
	if binding.ProviderIDs.TMDBID != first.Release.Identity.TMDBID {
		t.Fatal("fresh title retained a provisional identity binding")
	}
	...
	if store.commitCount() != 1 {
		t.Fatal("stale inherited title published a generation")
	}
	saved, err := store.LoadReleaseCorrections(t.Context(), source)
	if err != nil {
		t.Fatal(err)
	}
	if len(saved.Corrections.StaleContentFields) != 1 || saved.Corrections.Metadata.Title == nil {
		t.Fatal("stale title was not retained for confirmation")
	}
}
	...
	for _, change := range []string{"source", "category", "provider"} {
		t.Run(change, func(t *testing.T) {
			...
			if len(resolved.Corrections.Corrections.StaleContentFields) != 1 {
				t.Fatal("expected stale title before collection")
			}
			...
			if store.commitCount() != 1 {
				t.Fatal("stale title published a generation")
			}
		})
	}
}
