...
- Operation subjects contain finalized naming, media, episode, and identity facts. `Release` on these subjects is a canonical projection, not the mutable parser result. Use resolved media fields for technical decisions; keep naming tokens distinct from measured media. Defaults and fallbacks must use finalized facts; never reconstruct them from raw `rls` output, release names, filenames, or path extensions. When resolved facts cannot supply resolution, container, or type, preserve explicit tracker rejection or unknown-value mappings.
- Any direct metadata-provider override is allowed, including titles, years, AKA, original/localized titles, disambiguation, aliases, and descriptors. Prepared provider snapshots must match the source, identity, and generation. Preserve existing site precedence; resolve raw parser evidence upstream. Test conflicting parser/resolved values, missing facts, presentation omissions, and preserved provider overrides when changing resolution.
- Per-file resource enumeration may parse filenames for file-level episode membership; it must not replace resolved release-level facts.
