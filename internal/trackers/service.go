		...
		if s.repo != nil {
			for _, tracker := range trackers {
				...
				if err := s.repo.UpdateLatestUploadRecordStatus(ctx, meta.SourcePath, tracker, "failed"); err != nil {
					s.logger.Warnf("trackers: status update failed tracker=%s status=failed err=%v", tracker, err)
				}
			}
		}
	...
	if s.repo != nil {
		...
		updateRecordStatus := func(updateCtx context.Context, tracker string, status string) error {
			...
			if err := s.repo.UpdateLatestUploadRecordStatus(updateCtx, meta.SourcePath, trimmedTracker, trimmedStatus); err != nil {
				s.logger.Warnf("trackers: status update failed tracker=%s status=%s err=%v", trimmedTracker, trimmedStatus, err)
				recordMu.Lock()
...
func (s *Service) BuildPreparation(ctx context.Context, meta api.PreparedMetadata, trackersList []string) (api.PreparationPreview, error) {
	...
	preloaded, err := preloadDescriptionAssetData(ctx, meta, s.repo)
	if err != nil {
		s.logger.Warnf("trackers: preparation preload failed source=%s err=%v", meta.SourcePath, err)
		preloaded = nil
	}
	...
	if preflightUploaded {
		...
		if reloadErr != nil {
			s.logger.Warnf("trackers: preparation preload reload failed source=%s err=%v", meta.SourcePath, reloadErr)
		} else {
			preloaded = refreshed
		}
	}
	...
	for _, tracker := range resolved {
		...
		if !ok {
			...
			if err != nil {
				s.logger.Warnf("trackers: preparation image host resolution failed tracker=%s err=%v", tracker, err)
		...
		if err != nil {
			s.logger.Warnf("trackers: preparation assets failed tracker=%s err=%v", tracker, err)
...
func (s *Service) BuildUploadDryRun(ctx context.Context, meta api.PreparedMetadata, trackersList []string) ([]api.TrackerDryRunEntry, error) {
	...
	preloaded, err := preloadDescriptionAssetData(ctx, meta, s.repo)
	if err != nil {
		s.logger.Warnf("trackers: dry-run preload failed source=%s err=%v", meta.SourcePath, err)
		preloaded = nil
	}
	...
	for _, tracker := range resolved {
		...
		if err != nil {
			s.logger.Warnf("trackers: dry-run image host resolution failed tracker=%s err=%v", tracker, err)
			entry.Status = "error"
			entry.Message = err.Error()
			results = append(results, entry)
			continue
