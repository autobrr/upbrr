...
func TestCZTHandlerSearchUsesPasskeyAndParsesArray(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			meta: api.DuplicateSubject{Release: api.ReleaseInfo{Title: title}},
		},
		{
		...
		},
		{
			...
			cfg:  config.TrackerConfig{Passkey: " passkey123 "},
			meta: api.DuplicateSubject{ReleaseName: title},
		},
	}
...
func TestCZTHandlerSearchCancellationAfterResponseReturnsNoEntries(t *testing.T) {
	...
	entries, notes, err := adapterEvidence(handler.Search(ctx, api.DuplicateSubject{
		Release: api.ReleaseInfo{Title: "Movie"},
	}))
	if err == nil {
...
func TestCZTHandlerSearchCredentialMatrix(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			notes, err := result.Notes(), result.Cause()
			if tc.wantError != "" {
			...
			if result.Disposition() != dupe.DispositionNotRun || result.Code() != dupe.NotRunMissingCredentials ||
				!strings.Contains(result.SafeMessage(), tc.wantSkip) {
...
func TestCZTHandlerSearchMissingTitleSkipsWithoutHTTP(t *testing.T) {
	...
	if result.Disposition() != dupe.DispositionNotRun || result.Code() != dupe.NotRunMissingMetadata ||
		!strings.Contains(result.SafeMessage(), "missing title") {
		t.Fatalf("unexpected result disposition=%v code=%q message=%q", result.Disposition(), result.Code(), result.SafeMessage())
	}
	if called.Load() {
