...
func upload(ctx context.Context, req trackers.UploadRequest) (api.UploadSummary, error) {
	...
	return api.UploadSummary{}, fmt.Errorf("trackers: RTF %s", metautil.FirstNonEmptyTrimmed(decoded.Message, fmt.Sprintf("upload failed with status %d", resp.StatusCode)))
}
...
func prepareUploadState(ctx context.Context, req trackers.UploadRequest) (uploadState, error) {
	...
	payload := map[string]any{
		"name":        metautil.FirstNonEmptyTrimmed(req.Meta.ReleaseName, req.Meta.Release.Title, req.Meta.Filename),
