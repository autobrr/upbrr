...
func TestUploadKeepsRemoteSuccessWhenNoInjectableArtifactExists(t *testing.T) {
	...
	if len(result.UploadedTorrents) != 1 || result.UploadedTorrents[0].TorrentID != "123" ||
		result.UploadedTorrents[0].TorrentPath != "" || result.UploadedTorrents[0].DownloadURL != "" {
		t.Fatalf("expected artifact-less remote success entry, got %+v", result.UploadedTorrents)
	}
}
