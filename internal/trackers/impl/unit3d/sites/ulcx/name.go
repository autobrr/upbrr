...
func applyULCXTVDBDisambiguation(name string, meta api.UploadSubject) string {
	...
	title, alternate, tail, ok := splitULCXTVName(name, meta, evidence)
	if !ok {
		return name
	}
	locale := ""
...
func splitULCXTVName(name string, meta api.UploadSubject, evidence api.TVDBNameDisambiguation) (string, string, string, bool) {
	...
	remainder = trimULCXLeadingYear(remainder, evidence.SeriesYear)
	tailStart := findULCXTVTailStart(remainder, meta)
	if tailStart < 0 {
		return "", "", "", false
	}
...
func trimULCXLeadingYear(value string, year int) string {
	if year <= 0 {
		return value
	}
...
func findULCXTVTailStart(value string, meta api.UploadSubject) int {
	candidates := []string{
	...
	for _, candidate := range candidates {
		if index := findULCXNameElement(value, candidate); index >= 0 && (best < 0 || index < best) {
			best = index
		}
	}
	return best
