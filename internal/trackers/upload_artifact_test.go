...
func TestResolveTrackerTorrentArtifactPathPrefixesTrackerName(t *testing.T) {
	...
	got, err := ResolveTrackerTorrentArtifactPath(api.UploadSubject{SourcePath: sourcePath}, filepath.Join(tmp, "state", "upbrr.db"), "TEST")
	if err != nil {
		t.Fatalf("resolve tracker torrent artifact: %v", err)
	}
	if filepath.Base(got) != "[test].Example.Movie.2026.BluRay.1080p.DTS.x264-GRP.mkv.torrent" {
