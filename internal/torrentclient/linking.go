...
// linkStagingCleanup records the staged destination to remove when client
// injection fails and whether its now-empty tracker directory may also be removed.
...
// Run removes the staged destination after proving it is a child of TrackerDir.
// It removes TrackerDir only when requested and the directory is empty.
...
func (s *Service) prepareLinkStaging(
	...
	if torrentPath := strings.TrimSpace(torrent.Path); torrentPath != "" {
		...
		if !client.FallbackAllowed() {
			return linkStagingResult{}, fmt.Errorf(
				"clients: %s cannot validate %s staging for URL-only torrent; provide a torrent file or enable allow_fallback for this client",
				clientName,
				mode,
			)
		}
...
func hardlinkDirectory(ctx context.Context, source string, dest string) error {
	...
	if err := filepath.WalkDir(source, func(path string, entry fs.DirEntry, walkErr error) error {
		...
		//nolint:gosec // Hardlink staging intentionally links files from a user-selected source tree into a guarded destination.
