...
var imdbGoFormatPattern = regexp.MustCompile(`(?i)(?:tt%[0-9]*d|%07d|imdb\.com/title/tt%)`)

var imdbFrontendFormatPattern = regexp.MustCompile(`(?:tt\$\{[^\n]+\}|\.padStart\(\s*7\s*,\s*["']0["']\s*\))`)
...
func checkGoFile(path string, relative string) ([]Violation, error) {
	...
	ast.Inspect(parsed, func(node ast.Node) bool {
		...
		if !strings.HasPrefix(relative, "internal/providerid/") {
			...
			if binary, ok := node.(*ast.BinaryExpr); ok && binary.Op == token.ADD && adHocIMDbPrefixConcat(binary) {
				add(binary.Pos(), "numeric IMDb formatting belongs to internal/providerid")
			}
		}
		if call, ok := node.(*ast.CallExpr); ok {
...
func checkAdHocIMDbFormatCall(call *ast.CallExpr, add func(token.Pos, string)) {
	...
	if ok && identifier.Name == "formatOptionalInt" && directIMDbArgument(call.Args) {
		add(call.Pos(), "numeric IMDb formatting belongs to internal/providerid")
	}
}
...
func directIMDbArgument(arguments []ast.Expr) bool {
	for _, argument := range arguments {
		switch typed := argument.(type) {
		case *ast.Ident:
			if strings.HasSuffix(strings.ToLower(typed.Name), "imdbid") {
				return true
			}
		case *ast.SelectorExpr:
			if strings.HasSuffix(strings.ToLower(typed.Sel.Name), "imdbid") {
				return true
			}
		case *ast.ParenExpr:
			if directIMDbArgument([]ast.Expr{typed.X}) {
				return true
			}
		case *ast.StarExpr:
			if directIMDbArgument([]ast.Expr{typed.X}) {
...
func adHocIMDbPrefixConcat(binary *ast.BinaryExpr) bool {
	return stringLiteral(binary.X, "tt") && strconvIntegerFormatCall(binary.Y) ||
		stringLiteral(binary.Y, "tt") && strconvIntegerFormatCall(binary.X)
}

func stringLiteral(expression ast.Expr, want string) bool {
	literal, ok := expression.(*ast.BasicLit)
	return ok && literal.Kind == token.STRING && strings.Trim(literal.Value, "`\"") == want
}

func strconvIntegerFormatCall(expression ast.Expr) bool {
	call, ok := expression.(*ast.CallExpr)
	if !ok {
		return false
	}
	selector, ok := call.Fun.(*ast.SelectorExpr)
	return ok && selectedTypeName(selector.X) == "strconv" && (selector.Sel.Name == "Itoa" || selector.Sel.Name == "FormatInt")
}
...
func checkFrontendFile(path string, relative string) ([]Violation, error) {
	...
	if relative != "webui/src/utils/providerId.ts" {
		if match := imdbFrontendFormatPattern.FindStringIndex(text); match != nil {
			add(match[0], "numeric IMDb formatting belongs to webui/src/utils/providerId.ts")
		}
	}
